...
public:
	CGameObject();
	CGameObject(int nResource);
	virtual ~CGameObject();

public:
	char							m_pstrFrameName[256] = { 0 };
	...
	int								m_nMeshesInHierarchy = 0;
	ID3D12Resource**				m_ppd3dcbInstancingGameObjects = NULL;
	UINT8**							m_ppcbMappedInstancingGameObjects = NULL;
	int*							m_pnMaterialsInHierarchy = NULL;

	int								m_nMeshInHierarchy = 0;
	...
	virtual void Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera, ID3D12Resource** ppd3dcbGameObjects, UINT8** ppcbMappedGameObjects);
	...
	virtual void UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList, UINT8* pbMappedcbGameObject);
...
public:
	...
	static CGameObject *LoadFrameHierarchyFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, FILE *pInFile, int* pnMeshesInHierarchyd, int* pnMaterialsInHierarchy);
	static CGameObject *LoadGeometryFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, char *pstrFileName, int* pnMeshesInHierarchy, int* pnMaterialsInHierarchy);
